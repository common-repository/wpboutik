...
use NF\WPBOUTIK\Singleton;

class Block_Templates_Utils {
	const ELIGIBLE_FOR_ARCHIVE_PRODUCT_FALLBACK = array( 'taxonomy-product_cat', 'taxonomy-product_tag' );
	/**
	...
	/**
	 * wpboutik plugin slug
	 *
	 * This is used to save templates to the DB which are stored against this value in the wp_terms table.
	 *
	 * @var string
	 */
	const PLUGIN_SLUG = 'wpboutik/wpboutik';

	/**
	 * Deprecated wpboutik plugin slug
	 *
	 * For supporting users who have customized templates under the incorrect plugin slug during the first release.
	 * More context found here: https://github.com/wpboutik/wpboutik-gutenberg-products-block/issues/5423.
	 *
	 * @var string
	 */
	const DEPRECATED_PLUGIN_SLUG = 'wpboutik';
	...
	public static function build_template_result_from_post( $post ) {
		...
		// We are checking 'wpboutik' to maintain classic templates which are saved to the DB,
		// prior to updating to use the correct slug.
		// More information found here: https://github.com/wpboutik/wpboutik-gutenberg-products-block/issues/5423.
	...
	public static function build_template_result_from_file( $template_file, $template_type ) {
		...
		// If the theme has an archive-wpboutik_products.html template but does not have product taxonomy templates
		// then we will load in the archive-wpboutik_product.html template from the theme to use for product taxonomies on the frontend.
		...
		// Remove the term description block from the archive-wpboutik_product template
		// as the Product Catalog/Shop page doesn't have a description.
		if ( 'archive-wpboutik_product' === $template_file->slug ) {
	...
	public static function get_templates_directory( $template_type = 'wp_template' ) {
		$root_path                = WPBOUTIK_BLOCKS_TEMPLATES . DIRECTORY_SEPARATOR;
	...
	public static function get_plugin_block_template_types() {
		return array(
			'single-wpboutik_product'  => array(
				'title'       => _x( 'Single Product', 'Template name', 'wpboutik' ),
				'description' => __( 'Displays a single product.', 'wpboutik' ),
			),
			'archive-wpboutik_product' => array(
				'title'       => _x( 'Product Catalog', 'Template name', 'wpboutik' ),
				'description' => __( 'Displays your products.', 'wpboutik' ),
			),
			'taxonomy-product_cat'     => array(
				'title'       => _x( 'Products by Category', 'Template name', 'wpboutik' ),
				'description' => __( 'Displays products filtered by a category.', 'wpboutik' ),
			),
			'taxonomy-product_tag'     => array(
				'title'       => _x( 'Products by Tag', 'Template name', 'wpboutik' ),
				'description' => __( 'Displays products filtered by a tag.', 'wpboutik' ),
			),
		);
	}
	...
	public static function supports_block_templates( $template_type = 'wp_template' ) {
		if ( 'wp_template_part' === $template_type && ( wpb_current_theme_is_fse_theme() || current_theme_supports( 'block-template-parts' ) ) ) {
			return true;
		} elseif ( 'wp_template' === $template_type && wpb_current_theme_is_fse_theme() ) {
			return true;
		}
	...
	/**
	 * Checks if we can fall back to the `archive-wpboutik_product` template for a given slug.
	 *
	 * `taxonomy-product_cat`, `taxonomy-product_tag`, `taxonomy-product_attribute` templates can
	 *  generally use the `archive-wpboutik_product` as a fallback if there are no specific overrides.
	 *
	...
	/**
	 * Checks if we can fall back to an `archive-wpboutik_product` template stored on the db for a given slug.
	 *
	 * @param string $template_slug Slug to check for fallbacks.
	...
	public static function template_is_eligible_for_product_archive_fallback_from_db( $template_slug, $db_templates ) {
		...
		$array_filter = array_filter(
			...
			function ( $template ) use ( $template_slug ) {
				return 'archive-wpboutik_product' === $template->slug;
			}
		);
	...
	/**
	 * Gets the `archive-wpboutik_product` fallback template stored on the db for a given slug.
	 *
	 * @param string $template_slug Slug to check for fallbacks.
	 * @param array $db_templates Templates that have already been found on the db.
	 *
	...
	public static function get_fallback_template_from_db( $template_slug, $db_templates ) {
		...
		foreach ( $db_templates as $template ) {
			if ( 'archive-wpboutik_product' === $template->slug ) {
				return $template;
			}
		}
	...
	/**
	 * Checks if we can fall back to the `archive-wpboutik_product` file template for a given slug in the current theme.
	 *
	...
	public static function template_is_eligible_for_product_archive_fallback_from_theme( $template_slug ) {
		return self::template_is_eligible_for_product_archive_fallback( $template_slug )
		       ...
		       && self::theme_has_template( 'archive-wpboutik_product' );
	}
	...
	public static function remove_theme_templates_with_custom_alternative( $templates ) {
		...
		// for `wpboutik` in $template->source here because wpboutik templates won't have been added to $templates
	...
	public static function get_block_templates_from_db( $slugs = array(), $template_type = 'wp_template' ) {
		...
		$saved_wpb_templates = $check_query->posts;

		return array_map(
			function ( $saved_wpb_template ) {
				return self::build_template_result_from_post( $saved_wpb_template );
			},
			$saved_wpb_templates
		);
	}
...
use Automattic\wpboutik\Blocks\Templates\CartTemplate;
use Automattic\wpboutik\Blocks\Templates\CheckoutHeaderTemplate;
use Automattic\wpboutik\Blocks\Templates\CheckoutTemplate;
use Automattic\wpboutik\Blocks\Templates\MiniCartTemplate;
use Automattic\wpboutik\Blocks\Templates\OrderConfirmationTemplate;
use Automattic\wpboutik\Blocks\Templates\ProductAttributeTemplate;
use Automattic\wpboutik\Blocks\Templates\ProductSearchResultsTemplate;
