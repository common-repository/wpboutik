 ...
 * WPBoutik cart
 *
 * The WPBoutik cart class stores cart data and active coupons as well as handling customer sessions and some cart related urls.
...
require_once WPBOUTIK_DIR . 'classes/wpb-cart-session.php';
...
class WPB_Cart {
	...
	public function __construct() {
		$this->session = new WPB_Cart_Session( $this );

		// Register hooks for the objects.
		$this->session->init();

		add_action( 'wpboutik_add_to_cart', array( $this, 'calculate_totals' ), 20, 0 );
		add_action( 'woocommerce_applied_coupon', array( $this, 'calculate_totals' ), 20, 0 );
		add_action( 'wpboutik_cart_item_removed', array( $this, 'calculate_totals' ), 20, 0 );
		add_action( 'wpboutik_cart_item_restored', array( $this, 'calculate_totals' ), 20, 0 );
	...
	*/
	...
	public function get_cart_contents() {
		return apply_filters( 'wpboutik_get_cart_contents', (array) $this->cart_contents );
	}
	...
	public function get_subtotal() {
		return apply_filters( 'wpboutik_cart_' . __FUNCTION__, $this->get_totals_var( 'subtotal' ) );
	}
	...
	public function get_subtotal_tax() {
		return apply_filters( 'wpboutik_cart_' . __FUNCTION__, $this->get_totals_var( 'subtotal_tax' ) );
	}
	...
	public function get_discount_total() {
		return apply_filters( 'wpboutik_cart_' . __FUNCTION__, $this->get_totals_var( 'discount_total' ) );
	}
	...
	public function get_discount_tax() {
		return apply_filters( 'wpboutik_cart_' . __FUNCTION__, $this->get_totals_var( 'discount_tax' ) );
	}
	...
	public function get_shipping_total() {
		return apply_filters( 'wpboutik_cart_' . __FUNCTION__, $this->get_totals_var( 'shipping_total' ) );
	}
	...
	public function get_shipping_tax() {
		return apply_filters( 'wpboutik_cart_' . __FUNCTION__, $this->get_totals_var( 'shipping_tax' ) );
	}
	...
	public function get_cart_contents_total() {
		return apply_filters( 'wpboutik_cart_' . __FUNCTION__, $this->get_totals_var( 'cart_contents_total' ) );
	}
	...
	public function get_cart_contents_tax() {
		return apply_filters( 'wpboutik_cart_' . __FUNCTION__, $this->get_totals_var( 'cart_contents_tax' ) );
	}
	...
	public function get_total() {
		$total = apply_filters( 'wpboutik_cart_' . __FUNCTION__, $this->get_totals_var( 'total' ) );

		return apply_filters( 'wpboutik_cart_total', $total );
	}
	...
	public function get_total_tax() {
		return apply_filters( 'wpboutik_cart_' . __FUNCTION__, $this->get_totals_var( 'total_tax' ) );
	}
	...
	public function get_fee_total() {
		return apply_filters( 'wpboutik_cart_' . __FUNCTION__, $this->get_totals_var( 'fee_total' ) );
	}
	...
	public function get_fee_tax() {
		return apply_filters( 'wpboutik_cart_' . __FUNCTION__, $this->get_totals_var( 'fee_tax' ) );
	}
	...
	public function get_shipping_taxes() {
		return apply_filters( 'wpboutik_cart_' . __FUNCTION__, $this->get_totals_var( 'shipping_taxes' ) );
	}
	...
	/*public function get_cart_contents_taxes() {
		return apply_filters( 'wpboutik_cart_' . __FUNCTION__, $this->get_totals_var( 'cart_contents_taxes' ) );
	}*/
	...
	/*public function get_fee_taxes() {
		return apply_filters( 'wpboutik_cart_' . __FUNCTION__, $this->get_totals_var( 'fee_taxes' ) );
	}*/
	...
	public function display_prices_including_tax() {
		return apply_filters( 'wpboutik_cart_' . __FUNCTION__, 'incl' === $this->get_tax_price_display_mode() );
	}
	...
	*/
	...
	public function set_subtotal( $value ) {
		$this->totals['subtotal'] = wpb_format_decimal( $value );
	}
	...
	public function set_shipping_total( $value ) {
		$this->totals['shipping_total'] = wpb_format_decimal( $value );
	}
	...
	public function set_cart_contents_total( $value ) {
		$this->totals['cart_contents_total'] = wpb_format_decimal( $value );
	}
	...
	public function set_total( $value ) {
		$this->totals['total'] = wpb_format_decimal( $value, wpb_get_price_decimals() );
	}
	...
	public function set_total_tax( $value ) {
		...
		$this->totals['total_tax'] = wpb_round_tax_total( $value );
	}
	...
	public function set_fee_total( $value ) {
		$this->totals['fee_total'] = wpb_format_decimal( $value );
	}
	...
	*/
	...
	/*public function get_taxes() {
		return apply_filters( 'wpboutik_cart_get_taxes', wc_array_merge_recursive_numeric( $this->get_shipping_taxes(), $this->get_cart_contents_taxes(), $this->get_fee_taxes() ), $this );
	}*/
	...
	public function get_cart() {
		if ( ! did_action( 'wp_loaded' ) ) {
			//wc_doing_it_wrong( __FUNCTION__, __( 'Get cart should not be called before the wp_loaded action.', 'wpbotuik' ), '1.0' );
		}
		if ( ! did_action( 'wpboutik_load_cart_from_session' ) ) {
	...
	public function empty_cart( $clear_persistent_cart = true ) {
		...
		do_action( 'wpboutik_before_cart_emptied', $clear_persistent_cart );
		...
		do_action( 'wpboutik_cart_emptied', $clear_persistent_cart );
	}
	...
	public function get_cart_contents_count() {
		return apply_filters( 'wpboutik_cart_contents_count', array_sum( wp_list_pluck( $this->get_cart(), 'quantity' ) ) );
	}
	...
	public function get_cart_contents_weight() {
		...
		return apply_filters( 'wpboutik_cart_contents_weight', $weight );
	}
	...
	public function has_delivery_need() {
		foreach ( $this->get_cart() as $values ) {
			$product_type = get_post_meta( $values['product_id'], 'type', true );
			if ( $product_type != 'gift_card' && $product_type != 'virtual_product' && $product_type != 'plugin' && $product_type != 'abonnement' ) {
				return true;
			}
		}
	...
	public function allowed_payments () {
		$payments = [
			'card',
			'mollie',
			'monetico',
		 ...
		 foreach ( $this->get_cart() as $values ) {
			 ...
			 $type      = get_post_meta( $values['product_id'], 'type', true );
			 $recursive = get_post_meta( $values['product_id'], 'recursive', true );
			 if ( $type == 'abonnement' || ( $type == 'plugin' && $recursive ) ) {

				 $payments = [ 'card', 'bacs' ];

				 /*$licenses = get_post_meta( $values['product_id'], 'licenses', true );
				 $licenses = json_decode( $licenses );
				 if ( $values['variation_id'] != "0" ) {
					 $variants  = get_post_meta( $values['product_id'], 'variants', true );
					 $variation = wpboutik_get_variation_by_id( json_decode( $variants ), $values['variation_id'] );
					 if ( isset( $variation->recursive ) && $variation->recursive == 1 ) {
						foreach ( $licenses as $license ) {
							 if ( $license->variant_id == $variation->id ) {
								 if ( ! empty( $license->abonnement ) ) {
									 foreach ( $license->abonnement as $abo ) {
										 if ( $abo->paiement_type == 'paypal' ) {
											 $payments[] = 'paypal';
										 }
									 }
								 }
	...
	public function get_licenses () {
		foreach ( $this->get_cart() as $values ) {
			$type = get_post_meta( $values['product_id'], 'type', true );
			if ( $type == 'plugin' || $type == 'abonnement') {
				$licenses = get_post_meta( $values['product_id'], 'licenses', true );
				$licenses = json_decode($licenses);
				$visibles_licenses = [];
				if ($values['variation_id'] != "0") {
					...
					if (isset($variation->recursive) && $variation->recursive == 1) {
						foreach ($licenses as $license) {
							if ($license->variant_id == $variation->id) {
								$visibles_licenses[] = $license;
								break;
							}
						}
					}
				} else {
					if (get_post_meta( $values['product_id'], 'recursive', true ) == 1) {
						foreach ($licenses as $license) {
		...
		return isset($visibles_licenses) ? $visibles_licenses : [];
	}
	/**
	...
	/*public function check_cart_item_stock() {
		...
		foreach ( $this->get_cart() as $values ) {
			...
			if ( apply_filters( 'wpboutik_cart_item_required_stock_is_not_enough', $product->get_stock_quantity() < ( $held_stock + $required_stock ), $product, $values ) ) {
	...
	/*public function get_cross_sells() {
		...
		return apply_filters( 'wpboutik_cart_crosssell_ids', wp_parse_id_list( $cross_sells ), $this );
	}*/
	...
	/*public function get_tax_totals() {
		...
		foreach ( $taxes as $key => $tax ) {
			...
			if ( $code || apply_filters( 'wpboutik_cart_remove_taxes_zero_rate_id', 'zero-rated' ) === $key ) {
				...
				if ( isset( $shipping_taxes[ $key ] ) ) {
					...
					$tax = wpb_round_tax_total( $tax );
					$val = $shipping_taxes[ $key ];
					if ( ! is_numeric( $shipping_taxes[ $key ] ) ) {
						$val = floatval( $shipping_taxes[ $key ] );
					}
					$tax += round( $val, 2 );
					unset( $shipping_taxes[ $key ] );
				}
				$tax_totals[ $code ]->amount           += wpb_round_tax_total( $tax );
				$tax_totals[ $code ]->formatted_amount = wc_price( $tax_totals[ $code ]->amount );
			}
		}

		if ( apply_filters( 'wpboutik_cart_hide_zero_taxes', true ) ) {
			$amounts    = array_filter( wp_list_pluck( $tax_totals, 'amount' ) );
			$tax_totals = array_intersect_key( $tax_totals, $amounts );
		}

		return apply_filters( 'wpboutik_cart_tax_totals', $tax_totals, $this );
	}*/
	 ...
	 * @return string cart item key
	 */
	public function generate_cart_id( $product_id, $variation_id = 0, $variations = array(), $cart_item_data = array(), $customization = '' ) {
		...
		if ( is_array( $customization ) && ! empty( $customization ) ) {
			$cart_item_custom_key = '';
			foreach ( $customization as $key => $value ) {
				if ( is_array( $value ) || is_object( $value ) ) {
					$value = http_build_query( $value );
				}
				$cart_item_custom_key .= trim( $key ) . trim( $value );

			}
			$id_parts[] = $cart_item_custom_key;
		}

		return apply_filters( 'wpboutik_cart_id', md5( implode( '_', $id_parts ) ), $product_id, $variation_id, $variations, $cart_item_data );
	}
	 ...
	 */
	public function add_to_cart( $product_id = 0, $product_name = '', $quantity = 1, $variation_id = 0, $variations = array(), $cart_item_data = array(), $customization = null ) {
		//$product_data = wc_get_product( $variation_id ? $variation_id : $product_id );
		$quantity = apply_filters( 'wpboutik_add_to_cart_quantity', $quantity, $product_id );

		// Load cart item data - may be added by other plugins.
		$cart_item_data = (array) apply_filters( 'wpboutik_add_cart_item_data', $cart_item_data, $product_id, $variation_id, $quantity, $customization );

		// Generate a ID based on product ID, variation ID, variation data, and other cart item data.
		$cart_id = $this->generate_cart_id( $product_id, $variation_id, $variations, $cart_item_data, $customization );
			...
			$this->cart_contents[ $cart_item_key ] = apply_filters(
				...
				array_merge(
					...
					array(
						...
						//'data'         => $product_data,
						//'data_hash'    => wpb_get_cart_item_data_hash( $product_data ),
		...
		$this->cart_contents = apply_filters( 'wpboutik_cart_contents_changed', $this->cart_contents );

		do_action( 'wpboutik_add_to_cart', $cart_item_key, $product_id, $quantity, $variation_id, $variations, $cart_item_data );
	...
	public function remove_cart_item( $cart_item_key ) {
		if ( isset( $this->cart_contents[ $cart_item_key ] ) ) {
			...
			do_action( 'wpboutik_remove_cart_item', $cart_item_key, $this );

			unset( $this->cart_contents[ $cart_item_key ] );

			do_action( 'wpboutik_cart_item_removed', $cart_item_key, $this );
	...
	/*public function restore_cart_item( $cart_item_key ) {
		if ( isset( $this->removed_cart_contents[ $cart_item_key ] ) ) {
			...
			do_action( 'wpboutik_cart_item_restored', $cart_item_key, $this );
	...
	public function set_quantity( $cart_item_key, $quantity = 1, $refresh_totals = true ) {
		if ( 0 === $quantity || $quantity < 0 ) {
			//do_action( 'woocommerce_remove_cart_item', array( $cart_item_key, $this ) );
		 ...
		 * @param int $quantity contains the quantity of the item.
		 * @param WPB_Cart $this Cart class.
		 */
		do_action( 'wpboutik_cart_item_set_quantity', $cart_item_key, $quantity, $this );
	...
	public function calculate_totals() {
		...
		do_action( 'wpboutik_before_calculate_totals', $this );

		new WPB_Cart_Totals( $this );

		do_action( 'wpboutik_after_calculate_totals', $this );
	}
	...
	public function needs_payment() {
		return apply_filters( 'wpboutik_cart_needs_payment', 0 < $this->get_total(), $this );
	}
	...
	public function get_shipping_packages() {
		return apply_filters(
			'wpboutik_cart_shipping_packages',
			array(
				array(
					...
					'destination'     => array(
						...
						'address_1' => $this->get_customer()->get_shipping_address(),
	...
	/*public function needs_shipping() {
		...
		return apply_filters( 'wpboutik_cart_needs_shipping', $needs_shipping );
	}*/

	public function save_data_checkout( $datas ) {
		WPB()->session->set( 'data_user', $datas );
	}
	...
	/*public function needs_shipping_address() {
		return apply_filters( 'wpboutik_cart_needs_shipping_address', true === $this->needs_shipping() && ! wc_ship_to_billing_address_only() );
	}*/
	...
	/*public function show_shipping() {
		...
		return apply_filters( 'wpboutik_cart_ready_to_calc_shipping', true );
	}*/
	...
	/*public function get_cart_shipping_total() {
		...
		return apply_filters( 'wpboutik_cart_shipping_total', $total, $this );
	}*/
	...
	/*public function get_coupon_discount_amount( $code, $ex_tax = true ) {
		...
		return wc_cart_round_discount( $discount_amount, wpb_get_price_decimals() );
	}*/
	...
	/*public function get_coupon_discount_tax_amount( $code ) {
		...
		return wc_cart_round_discount( isset( $totals[ $code ] ) ? $totals[ $code ] : 0, wpb_get_price_decimals() );
	}*/
	...
	/*public function remove_coupon( $coupon_code ) {
		...
		do_action( 'wpboutik_removed_coupon', $coupon_code );
	...
	/*public function calculate_fees() {
		do_action( 'wpboutik_cart_calculate_fees', $this );
	}*/
	 ...
	 * wpboutik_cart_calculate_fees action during cart/checkout. Fees do not
	 * persist.
	 *
	...
	/*public function get_total_ex_tax() {
		return apply_filters( 'wpboutik_cart_total_ex_tax', wc_price( max( 0, $this->get_total( 'edit' ) - $this->get_total_tax() ) ) );
	}*/
	...
	/*public function get_cart_total() {
		return apply_filters( 'wpboutik_cart_contents_total', wc_price( wc_prices_include_tax() ? $this->get_cart_contents_total() + $this->get_cart_contents_tax() : $this->get_cart_contents_total() ) );
	}*/
	...
	/*public function get_cart_subtotal( $compound = false ) {
		...
		return apply_filters( 'wpboutik_cart_subtotal', $cart_subtotal, $compound, $this );
	}*/
	...
	/*public function get_product_price( $product ) {
		...
		return apply_filters( 'wpboutik_cart_product_price', wc_price( $product_price ), $product );
	}*/
	...
	/*public function get_product_subtotal( $product, $quantity ) {
		...
		return apply_filters( 'wpboutik_cart_product_subtotal', $product_subtotal, $product, $quantity, $this );
	}*/
	...
	/*public function get_cart_tax() {
		$cart_total_tax = wpb_round_tax_total( $this->get_cart_contents_tax() + $this->get_shipping_tax() + $this->get_fee_tax() );
	...
	/*public function get_taxes_total( $compound = true, $display = true ) {
		...
		if ( $display ) {
			$total = wpb_format_decimal( $total, wpb_get_price_decimals() );
		}

		return apply_filters( 'wpboutik_cart_taxes_total', $total, $compound, $display, $this );
	}*/
	...
	/*public function get_total_discount() {
		return apply_filters( 'wpboutik_cart_total_discount', $this->get_discount_total() ? wc_price( $this->get_discount_total() ) : false, $this );
	}*/
	...
	private function reset_totals() {
		$this->totals = $this->default_totals;
		//$this->fees_api->remove_all_fees();
		do_action( 'wpboutik_cart_reset', $this, false );
	}
	...
	public function get_tax_price_display_mode() {
		//&& $this->get_customer()->get_is_vat_exempt()
		return '';
	...
	public function get_cart_hash() {
		...
		return apply_filters( 'wpboutik_cart_hash', $hash, $cart_session );
	}
}
