...
class WPB_Cache_Helper {
	...
	public static function get_cache_prefix( $group ) {
		// Get cache key - uses cache key wpb_orders_cache_prefix to invalidate when needed.
	...
	public static function invalidate_cache_group( $group ) {
		return wp_cache_set( 'wpb_' . $group . '_cache_prefix', microtime(), $group );
	}
	...
	public static function additional_nocache_headers( $headers ) {
		 ...
		 */
		if ( apply_filters( 'wpboutik_enable_nocache_headers', false ) ) {
			$set_cache = true;
		}
		...
		if ( false !== strpos( $agent, 'Chrome' ) && isset( $wp_query ) && is_page( wpboutik_get_page_id( 'cart' ) ) ) {
			$set_cache = true;
		}
	...
	public static function invalidate_attribute_count( $attribute_keys ) {
		if ( $attribute_keys ) {
			foreach ( $attribute_keys as $attribute_key ) {
				self::queue_delete_transient( 'wpb_layered_nav_counts_' . $attribute_key );
			}
		}
	}
	...
	public static function prevent_caching() {
		...
		$page_ids = array_filter( array(
			wpboutik_get_page_id( 'cart' ),
			wpboutik_get_page_id( 'checkout' ),
			wpboutik_get_page_id( 'account' )
		) );
	...
	/*public static function geolocation_ajax_redirect() {
		if ( 'geolocation_ajax' === get_option( 'wpboutik_default_customer_address' ) && ! is_page( wpboutik_get_page_id( 'checkout' ) ) && ! is_page( wpboutik_get_page_id( 'cart' ) ) && ! is_page( wpboutik_get_page_id( 'account' ) ) && ! wp_doing_ajax() && empty( $_POST ) ) { // WPCS: CSRF ok, input var ok.
			$location_hash = self::geolocation_ajax_get_location_hash();
			$current_hash  = isset( $_GET['v'] ) ? wpb_clean( wp_unslash( $_GET['v'] ) ) : ''; // WPCS: sanitization ok, input var ok, CSRF ok.
	...
	/*public static function update_geolocation_hash() {
		if ( 'geolocation_ajax' === get_option( 'wpboutik_default_customer_address' ) ) {
			wpb_setcookie( 'wpboutik_geo_hash', static::geolocation_ajax_get_location_hash(), time() + HOUR_IN_SECONDS );
		}
	}*/
	...
	public static function notices() {
		...
		if ( $enabled && ! in_array( '_wpb_session_', $settings, true ) ) {
			?>
            <div class="error">
                <p>
					<?php
					/* translators: 1: key 2: URL */
					echo wp_kses_post( sprintf( __( 'In order for <strong>database caching</strong> to work with WPboutik you must add %1$s to the "Ignored Query Strings" option in <a href="%2$s">W3 Total Cache settings</a>.', 'woocommerce' ), '<code>_wpb_session_</code>', esc_url( admin_url( 'admin.php?page=w3tc_dbcache' ) ) ) );
					?>
                </p>
            </div>
			<?php
	...
	public static function delete_version_transients( $version = '' ) {
		if ( ! wp_using_ext_object_cache() && ! empty( $version ) ) {
			...
			$limit = apply_filters( 'wpboutik_delete_version_transients_limit', 1000 );
